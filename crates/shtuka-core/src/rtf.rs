//! RTF diff for SAS-generated clinical tables. We parse the RTF into a styled
//! table model (cells with text + shading/borders/font/alignment, real column
//! widths from \cellx), align rows and cells between the two files, and tag each
//! cell added/removed/modified/equal. The frontend renders both sides as HTML
...
pub struct CellStyle {
    ...
    #[serde(rename = "widthPct", default, skip_serializing_if = "is_zero_f32")]
    pub width_pct: f32,
...
pub struct DiffCell {
    ...
    pub a_segs: Vec<crate::myers::Seg>,
    #[serde(rename = "bSegs", default, skip_serializing_if = "Vec::is_empty")]
    pub b_segs: Vec<crate::myers::Seg>,
}
        ...
        let status = match row_status {
            ...
            _ => {
                ...
                if ta != tb {
                    ...
                    if let Some((sa, sb)) = crate::text::inline_segments(ta, tb) {
                        a_segs = sa;
                        b_segs = sb;
                    }
                    "modified"
